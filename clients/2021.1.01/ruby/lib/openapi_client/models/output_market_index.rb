  ...
  class OutputMarketIndex
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::OutputMarketIndex` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::OutputMarketIndex`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'adj_open')
        self.adj_open = attributes[:'adj_open']
      end

      if attributes.key?(:'adj_high')
        self.adj_high = attributes[:'adj_high']
      end

      if attributes.key?(:'adj_low')
        self.adj_low = attributes[:'adj_low']
      end

      if attributes.key?(:'adj_close')
        self.adj_close = attributes[:'adj_close']
      end

      if attributes.key?(:'adj_volume')
    ...
    def hash
      [date, symbol, exchange, open, high, low, close, volume, adj_open, adj_high, adj_low, adj_close, adj_volume].hash
    end
