...
module OpenapiClient
  class OutputDateDifference
    ...
    # Total Years
    attr_accessor :total_years

    # Total Months
    attr_accessor :total_months
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::OutputDateDifference` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::OutputDateDifference`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'total_years')
        self.total_years = attributes[:'total_years']
      end

      if attributes.key?(:'total_months')
        self.total_months = attributes[:'total_months']
      end
    ...
    def hash
      [years, months, days, hours, minutes, milliseconds, total_years, total_months, total_days, total_hours, total_minutes, total_seconds, total_milliseconds, ticks].hash
    end
